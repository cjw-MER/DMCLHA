...
def build_tf_xlnet_to_pytorch_map(model, config, tf_weights=None):#创建一个从TensorFlow(TF)到PyTorch模块的映射，简化将TensorFlow XLNet模型装换为Pyorch模型的过程
    """ A map of modules from TF to PyTorch.
        I use a map to keep the PyTorch model as
        identical to the original PyTorch model as possible.
    """
    tf_to_pt_map = {}#初始化一个空字典，存储TF和PyTorch模块之间的映射关系
