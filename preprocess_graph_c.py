...
if __name__ == '__main__':
    ...
    relation_matrix = {}#初始化关系矩阵，存储节点之间的关系，键：节点对，值：节点之间的关系
    meet_matrix = {}
    relation_map = {'friend':1,'enemy':2,'neutral':0,'interact':3}#将关系标签映射到数字编码，interact表示相互作用
    ...
    #train = pd.concat([data1,data2,data3,data4], ignore_index=True)
    ...
    for p in enumerate(zip(child,parent,relations)):#遍历三元组
        ...
        if (unique_nodes_mapping[p[0]],unique_nodes_mapping[p[1]]) in relation_matrix:
            relation_matrix[(unique_nodes_mapping[p[0]],unique_nodes_mapping[p[1]])] += p[2]-1#表示了关系的强度
    ...
    for u in (unique_nodes_mapping.values()):
        concept_net[u] = []#对于每个索引值u，将其作为concept_net字典的键，并将对应值设置为空列表[]
        concept_net_c[u] = []
    for key in relation_matrix.keys():#遍历relation_matrix字典的所有键，即节点对
        if relation_matrix[key] > 0:#原关系为敌人
            ...
            #对比图
            if random.random()>0.1:
                triplets_c.append((key[0],1,key[1]))
                concept_net_c[key[0]].append([key[0],1,key[1]])
            else:
                triplets_c.append((key[0],3,key[1]))
                concept_net_c[key[0]].append([key[0],3,key[1]])
            ...
            if random.random()>0.1:
                ...
                triplets.append((key[0],3,key[1]))#双向边
                concept_net[key[0]].append([key[0],3,key[1]])

            #对比图
            if random.random()>0.1:
                triplets_c.append((key[0],2,key[1]))#把中立变为敌人关系
                concept_net_c[key[0]].append([key[0],2,key[1]])
            else: 
                triplets_c.append((key[0],3,key[1]))#把中立变为敌人关系
                concept_net_c[key[0]].append([key[0],3,key[1]])
        else:#原关系为朋友
            if random.random()>0.1:
            ...
            #对比图
            if random.random()>0.1:
                triplets_c.append((key[0],0,key[1]))#把朋友关系变为中立
                concept_net_c[key[0]].append([key[0],0,key[1]])
            else: 
                triplets_c.append((key[0],3,key[1]))#把朋友关系变为中立
                concept_net_c[key[0]].append([key[0],3,key[1]])
    ...
    #处理测试集中的数据，特别是对于那些在训练集中没有出现过的节点对
    i = 0
    for p in enumerate(zip(child,parent,relations)):
        ...
        if not ((unique_nodes_mapping[p[0]],unique_nodes_mapping[p[1]]) in relation_matrix):
            ...
            triplets_c.append((unique_nodes_mapping[p[0]],3,unique_nodes_mapping[p[1]]))
            ...
            concept_net_c[unique_nodes_mapping[p[0]]].append((unique_nodes_mapping[p[0]],3,unique_nodes_mapping[p[1]]))
            i+=1
    triplets = np.array(triplets)#将triplets列表转换为NumPy数组
    triplets_c = np.array(triplets_c)#将triplets列表转换为NumPy数组
    ...
    pickle.dump(concept_net_c, open('utils/concept_n_graphs_c_bert.pkl', 'wb'))
    np.ndarray.dump(triplets, open('utils/triplets_n_bert.np', 'wb'))
    np.ndarray.dump(triplets_c, open('utils/triplets_n_c_bert.np', 'wb'))
